...
public:
    MatchUploader(const std::string& url = "http://localhost:3000") : server_url(url) {}
    ...
    bool uploadMatch(const std::string& filename, 
        ...
        auto res = client.Post("/upload_match", headers, body, "multipart/form-data; boundary=" + boundary);
    ...
    void uploadMatchAsync(const std::string& filename,
                          ...
                          int duration_seconds) {
        std::thread([=]() {
            uploadMatch(filename, player1_name, player2_name, winner_name, duration_seconds);
        }).detach();
    }
};
